...
static void *serial_slip_read_thread(void *param)
{
    ...
    // Upewnij się, że deskryptor jest w trybie nieblokującym.
    ...
    while (!ctx->stopThread){
        if (ctx->connected) {
            ...
            if (bytesRead > 0) {
                // Przetwarzamy odebrane dane
                // Przykładowe wyświetlenie liczby odebranych bajtów
                // printf("Received %zd bytes\n", bytesRead);
                process_chunk(ctx, buffer, (size_t)bytesRead); // Twoja funkcja do obróbki danych
            } else if (bytesRead < 0) {
                // Sprawdzamy, czy zwracany błąd nie wynika z braku danych lub przerwania
                if (errno != EAGAIN && errno != EINTR) {
                    fprintf(stderr, "Read error (error %s)\n", strerror(errno));
                }
                // Jeśli errno wskazuje na brak danych, nic nie robimy i przechodzimy do opóźnienia
            }
        }
        // Odczekaj 1 ms, aby ograniczyć busy polling
        usleep(1000); // 1000 mikrosekund = 1 ms
    }
    return NULL;
}
